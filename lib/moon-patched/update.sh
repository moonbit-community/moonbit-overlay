...
v8_version=$(cat $cargo_lock | $tomlq --raw-output '.package[] | select(.name == "v8") | .version')
...
n2_version=$(cat $cargo_lock | $tomlq --raw-output '.package[] | select(.name == "n2") | .version')
n2_rev=$(cat $cargo_lock | $tomlq --raw-output '.package[] | select(.name == "n2") | .source' | $sednr 's|.*rev=(\w*)[^$].*|\1|p')
